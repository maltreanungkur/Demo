...
A natural basis for \\( R \\) is \\( \\{ \\overline{1}, \\overline{x^1}, \\dots, \\overline{x^{n-1}} \\} \\). Now, look at the set \\( \\{ \\overline{x^{p_1}}, \\overline{x^{p_2}}, \\dots, \\overline{x^{p_{n+1}}} \\} \\), this is an \\( n+1 \\)-element set inside an \\( n \\)-dimensional vector space, that's one more than the \\( \\dim R \\), so it *has* to be linearly dependent! That means there exist some \\( u_i \\in \\mathbb{Q} \\) such that  
